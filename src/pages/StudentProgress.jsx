          ...
          <>
            ...
            {Object.keys(bySubject).length > 0 && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8">
                <h3 className="text-base font-bold text-slate-900 tracking-tight mb-4">Performance by subject</h3>
                  ...
                  {Object.entries(bySubject).map(([subj, data], i) => {
                    ...
                    return (
                      <motion.div key={subj} variants={staggerItem}>
                        <Card padding="p-5" hoverable className="border-slate-200">
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <div className="text-sm font-bold text-slate-900 mb-1">{subj}</div>
                              <div className="text-xs text-slate-500 font-medium">{data.count} assignment{data.count !== 1 ? 's' : ''}</div>
                            </div>
                            <div className="text-2xl font-bold text-slate-900 font-mono tracking-tight">{pct}%</div>
                          </div>
                          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                            <motion.div 
                              ...
                              className={`h-full rounded-full ${pct >= 80 ? 'bg-emerald-500' : pct >= 60 ? 'bg-sky-500' : 'bg-red-500'}`} 
                            />
                          </div>
                        </Card>
                      </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
              <h3 className="text-base font-bold text-slate-900 tracking-tight mb-4">All assignments</h3>
